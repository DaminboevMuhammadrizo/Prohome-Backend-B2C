...
import { PropertyType, DealType, JobStatus, RealEstateStatus } from '@prisma/client';
...
export class DashboardService {
    ...
    async getMastersStats() {
        ...
        return {
            ...
            rating:
                typeof ratingAgg._avg.rating === 'number'
    ...
    async getTopMasters(limit = 10) {
        ...
        return masters.map((m) => ({
            ...
            avgRating:
                m.ratings.length > 0
                    ? +(m.ratings.reduce((s, r) => s + r.rating, 0) / m.ratings.length).toFixed(1)
                    : 0,
    ...
    async getRealEstateStats() {
        const [forSale, forRent, byType, totalViews, recentlyAdded] = await Promise.all([
            this.prisma.realEstate.count({
    ...
    async getSearchSuggestions(query: string) {
        ...
        return Array.from(new Map(suggestions.map((s) => [s.text, s])).values()).slice(0, 7);
    }
    ...
    async getPlatformOverview() {
        ...
        const [
            ...
            this.prisma.realEstate.count(),
            this.prisma.realEstate.count({ where: { status: RealEstateStatus.ACTIVE } }),
            this.prisma.realEstate.count({ where: { status: RealEstateStatus.SOLD } }),
            this.prisma.master.count(),
            this.prisma.master.count({ where: { isFree: true } }),
        ...
        return {
            ...
            engagement: {
                totalRatings,
                avgRating: avgRating._avg.rating ? +avgRating._avg.rating.toFixed(1) : 0,
