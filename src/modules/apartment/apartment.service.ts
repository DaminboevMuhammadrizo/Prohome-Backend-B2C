...
export class ApartmentService {
    ...
    async create(
        ...
        if (!images?.length) {
            throw new BadRequestException('Apartment uchun kamida bitta rasm majburiy');
        }
    ...
    async update(
        ...
        return this.prisma.apartment.update({
            ...
            data: {
                ...dto,
                images: replaceImages(images, apartment.images, this.config),
            },
        });
    }
    ...
    async updateStatus(id: number, user: AuthUser, dto: UpdateApartmentStatusDto) {
        ...
        if (apartment.sellerId)
            assertOwnership(apartment.sellerId, user, 'Siz faqat ozingizning apartmentingiz statusini ozgartira olasiz');

        return this.prisma.apartment.update({
            where: { id },
            data: {
                dealStatus: dto.dealStatus,
                soldAt: dto.dealStatus === ApartmentDealStatus.SOLD ? new Date() : null,
            },
        });
    }
