...
export class NotificationService {
    constructor(
        private readonly prisma: PrismaService, private readonly firebaseService: FirebaseService) { }
    ...
    private async findOwnedDeviceOrThrow(userId: number, deviceId: number) {
        const device = await this.prisma.userDevice.findUnique({ where: { id: deviceId } });
        if (!device) throw new NotFoundException('Qurilma topilmadi');

        if (device.userId !== userId) throw new ForbiddenException('Bu qurilma sizga tegishli emas');
        return device;
    }
    ...
    async registerDeviceForUser(userId: number, dto?: Partial<RegisterDeviceDto>) {
        if (!dto?.deviceId || !dto?.fcmToken) return null;
        await this.ensureUserExists(userId);

        return this.prisma.userDevice.upsert({
            where: {
                userId_deviceId: { userId, deviceId: dto.deviceId.trim() }
            },
            update: {
    ...
    async registerMyDevice(user: AuthUser, dto: RegisterDeviceDto) {
        ...
        return { message: 'Qurilma saqlandi', firebaseConfigured: this.firebaseService.isConfigured(), device };
    }
    ...
    async removeMyDevice(user: AuthUser, deviceId: number) {
        ...
        return { message: 'Qurilma ochirildi' };
    }
    ...
    async markAsRead(user: AuthUser, id: number) {
        ...
        return this.prisma.notification.update({
            where: { id }, data: { isRead: true, readAt: new Date() }
        });
    }
    ...
    async markAllAsRead(user: AuthUser) {
        ...
        return { message: 'Barcha notification oqilgan deb belgilandi', updatedCount: result.count };
    }

    async deleteMyNotification(user: AuthUser, id: number) {
        await this.getMyNotificationById(user, id);
        await this.prisma.notification.delete({ where: { id } });
        return { message: 'Notification ochirildi' };
    }
}
