...
export class PhoneIdentityService {
    ...
    async ensurePhoneAvailable(params: { phone: string; excludeUserId?: number; excludeCompanyId?: number }): Promise<string> {
            ...
            this.prisma.company.findFirst({
                where: {
                    ...
                    NOT: params.excludeCompanyId
                        ? { id: params.excludeCompanyId } : undefined,
                },
            }),
        ]);

        if (user || company)
            throw new ConflictException('Bu telefon raqam user yoki company uchun allaqachon band');
